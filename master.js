...
setInterval(function () {
	...
	for (let i = 0; i < networkposters.length; i++) {
		if (networkposters[i].count > 0 && mechanicuslevel > 1) {
	...
	for (let i = 0; i < upgradebleposters.length; i++) {
		...
		if (element.revealed === true) {
			html += "<fieldset><legend>" + element.name + " Upgrades</legend>" +
				...
				"Current Level: " + element.level + " Costs: " + (element.level * element.c1rate * element.p1rate) + emojify(element.c1type) +
				"<br>Upgrading will improve;<br>" +
				element.power1 + " (+" + element.power1l + ")" + " " + element.power1txt + "<br>";
			if (element.power2 !== "none") {
				html += element.power2 + " (+" + element.power2l + ")" + " " + element.power2txt + "<br>";
			}
			html += "</fieldset>";
		}
	}
...
function upgradeclick(i) {
	var c = upgradebleposters[i];
	var t = {
		c1type: c.c1type,
		c1rate: c.c1rate * c.level * c.p1rate,
		c2type: 0,
	}
...
function getmechanicuscost() {
	...
	switch (true) {
		case mechanicuslevel <= 50:
			mt.c1type = "internets";
			mt.c1rate = 100 * mechanicuslevel;
			mt.c2type = 0
			mt.c2rate = 0
			break;
			break;
		case 50 < mechanicuslevel <= 100:
			mt.c1type = "internets";
			mt.c1rate = 5000 * mechanicuslevel;
			mt.c2type = "science";
			mt.c2rate = 100 * mechanicuslevel;
			break;
		default:
